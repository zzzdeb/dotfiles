...
def gadateToGcaldate(date):
    ...
    tmp = calDay.split('.')
    nCalDay = str(NOW.year)+'/'+tmp[1]+'/'+tmp[0]
    return datetime.datetime.strptime(nCalDay)
    ...
    for schichtRow in driver.find_elements_by_xpath('//div[@class="card-row"]'):
        for day, group in enumerate(schichtRow.find_elements_by_xpath('div[@class="card-cell card-cell-employee"]')):
            ...
            #  -   div 'card-header card-header-employee' contains 'Zolboo Erdenebayar'
                ...
                genname = calDays[day].replace('/',NAMESEPSTR)+NAMESEPSTR+start.text.replace(':',NAMESEPSTR)+NAMESEPSTR+end.text.replace(':',NAMESEPSTR)
                p = sp.Popen(['gcalcli', 'search', genname], stdin=PIPE, stdout=PIPE, stderr=PIPE,
                            universal_newlines=True)
                gcalRet = p.communicate()[0]
                FROM=calDays[day]
                TODATE=start.text+'-'+end.text
                DURATION = FROM - TILL
                ...
                p = sp.Popen(['gcalcli', '--calendar', 'Job', 'add', '--title', genname, '--where', WHERE, '--when', str(FROM),'--duration', str(DURATION), '--noprompt'], stdin=PIPE, stdout=PIPE, stderr=PIPE,
                            universal_newlines=True)
